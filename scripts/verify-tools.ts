...
async function verifyTool(tool: ZeroKeyTool): Promise<VerificationResult> {
  ...
  try {
    ...
    const response = await fetch(tool.url, {
      ...
      headers: {
        'User-Agent': 'BanalAI-ToolVerifier/1.0 (https://github.com/sakurablush/banal)',
      },
      redirect: 'follow',
    });
...
async function verifyAllTools(): Promise<VerificationReport> {
  ...
  if (lastVerifiedCoverage.withoutDate > 0) {
    console.log(`   ${lastVerifiedCoverage.withoutDate} tools missing lastVerified date\n`);
  }
...
function writeDatedSummary(report: VerificationReport): void {
  ...
  const avgResponseTimeMs =
    report.results.length > 0
      ? Math.round(
          report.results.reduce((sum, r) => sum + r.responseTime, 0) / report.results.length
        )
      : 0;
  ...
  const summary: VerificationSummary = {
    ...
    successRate:
      report.totalTools > 0 ? Math.round((report.verified / report.totalTools) * 1000) / 1000 : 0,
