...
describe('ZeroKeyTool Validation', () => {
  ...
  it('should detect invalid URL', () => {
    ...
    const result = validateZeroKeyTool(tool);
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => e.field === 'url' && e.message.includes('not valid'))).toBe(true);
  });
  ...
  it('should warn about too many badges', () => {
    ...
    const result = validateZeroKeyTool(tool);
    expect(result.warnings.some(w => w.field === 'badges' && w.message.includes('too many'))).toBe(true);
  });
});
...
describe('AIModel Validation', () => {
  ...
  it('should warn about missing inference providers', () => {
    const model = { ...aiModels[0], availability: { ...aiModels[0].availability, inferenceProviders: [] } };
    const result = validateAIModel(model);
...
describe('ToolStack Validation', () => {
  ...
  it('should warn about too few tools', () => {
    ...
    const result = validateToolStack(stack);
    expect(result.warnings.some(w => w.field === 'tools' && w.message.includes('fewer than 3'))).toBe(true);
  });
...
describe('Helper Functions', () => {
  ...
  it('should generate validation report', () => {
    const result = {
      valid: false,
      errors: [
        { field: 'id', message: 'ID is required', severity: 'error' as const },
      ],
      warnings: [
        { field: 'badges', message: 'No badges', severity: 'warning' as const },
      ],
    };
