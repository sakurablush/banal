...
describe('empathy & MINDSET-voice quality smoke tests (non-regression)', () => {
  ...
  it('templates are free of the forbidden MINDSET-violating register', () => {
    ...
    expect(sanitized, 'no toxic positivity').not.toMatch(
      /\bdon.?t worry\b|\byou got this\b|\bjust try harder\b/
    );
    ...
    expect(sanitized, 'no "phone with N% battery" poverty framing').not.toMatch(
      /phone with \d+% battery/
    );
  });
...
describe('production readiness (strictness + no side effects)', () => {
  ...
  it('templates never contain runtime deps or dangerous patterns (smoke)', () => {
    ...
    expect(joined, 'no paid promotion').not.toMatch(
      /patreon|buy me a|premium|upgrade|subscribe \$/i
    );
  });
});
