...
describe('Progressive Filter', () => {
  ...
  describe('applyProgressiveFilters', () => {
    ...
    it('should filter by multiple tags', () => {
      ...
      expect(filtered.every((t) => t.requiresSignup === false && t.surface === 'web')).toBe(true);
    });
    ...
    it('should combine category and tag filters', () => {
      ...
      expect(filtered.every((t) => t.category === 'ai-chat' && t.requiresSignup === false)).toBe(
        true
      );
    });
    ...
    it('should filter by self-hosted tag', () => {
      ...
      const filtered = applyProgressiveFilters(zeroKeyTools, state);
      expect(filtered.every((t) => t.access === 'self-host' || t.access === 'open-source')).toBe(
        true
      );
    });
    ...
    it('should filter by free-forever tag', () => {
      ...
      const filtered = applyProgressiveFilters(zeroKeyTools, state);
      expect(filtered.every((t) => t.access === 'no-login' || t.access === 'open-source')).toBe(
        true
      );
    });
