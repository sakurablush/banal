...
describe('zero-key panel rendering', () => {
  ...
  it('report menu opens GitHub issue URL from menu item', () => {
    const root = renderPanel();
    ...
    const issueItem = root.querySelector(
      '.zk2-card-report-menu-item[data-action="issue"]'
    ) as HTMLButtonElement;
  ...
  it('report menu opens GitHub discussions URL from menu item', () => {
    const root = renderPanel();
    const mockOpen = vi.fn();
    window.open = mockOpen;
    ...
    const discussItem = root.querySelector(
      '.zk2-card-report-menu-item[data-action="discuss"]'
    ) as HTMLButtonElement;
    expect(discussItem).not.toBeNull();
    discussItem.click();

    expect(mockOpen).toHaveBeenCalledWith(
      expect.stringContaining('github.com/sakurablush/banal/discussions/new'),
      '_blank',
      'noopener,noreferrer'
    );
  });
  ...
  it('footer has Report menu on left and Open button on right', () => {
    const root = renderPanel();
    const footer = root.querySelector('.zk2-card-footer') as HTMLElement;
    const reportWrap = footer!.querySelector('.zk2-card-report-wrap');
    const reportBtn = footer!.querySelector('.zk2-card-report');
    const cta = footer!.querySelector('.zk2-card-cta');

    expect(footer).not.toBeNull();
    expect(reportWrap).not.toBeNull();
    expect(reportBtn).not.toBeNull();
    expect(cta).not.toBeNull();
    expect(footer!.children[0]).toBe(reportWrap);
