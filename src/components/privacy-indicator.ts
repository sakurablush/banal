...
export function renderPrivacyIndicator(privacy: PrivacyInfo, lang: Lang): HTMLElement {
  ...
  const levelText =
    privacy.level === 'high' ? copy.high : privacy.level === 'medium' ? copy.medium : copy.low;
...
function createPrivacyItem(icon: string, text: string, isPositive: boolean): HTMLElement {
  const item = create(
    'li',
    `privacy-details-item ${isPositive ? 'privacy-positive' : 'privacy-negative'}`
  );
