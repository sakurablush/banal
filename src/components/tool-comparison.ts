...
export function renderToolComparison(tools: Tool[], lang: Lang): HTMLElement {
...
function renderToolColumn(tool: Tool, copy: typeof COPY.en): HTMLElement {
  ...
  if (tool.quality?.trust?.githubStars) {
    column.appendChild(
      renderComparisonRow(copy.stars, formatStars(tool.quality.trust.githubStars))
    );
  }
  ...
  if (tool.quality?.privacy) {
    ...
    const privacyLabel =
      privacyLevel === 'high' ? copy.high : privacyLevel === 'medium' ? copy.medium : copy.low;
