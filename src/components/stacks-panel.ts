...
function renderStackCard(
  ...
  if (isCustom) {
    const badge = span(
      'stack-custom-badge',
      typeof copy.customBadge === 'string' ? copy.customBadge : 'Custom'
    );
  ...
  meta.appendChild(
    span(
      'stack-meta-item',
      `${typeof copy.budget === 'string' ? copy.budget : 'Budget'}: ${stack.cost.total}`
    )
  );
...
export function renderStacksPanel(container: HTMLElement, options: { lang: Lang }): StacksPanelApi {
  const { lang } = options;
  ...
  audienceLabel.textContent =
    typeof copy.browseByAudience === 'string' ? copy.browseByAudience : 'Browse by Audience';
  audienceRow.appendChild(audienceLabel);

  const chips = create('div', 'stacks-audience-chips');

  // All chip
  const allChip = create(
    'button',
    `stacks-audience-chip${state.audienceFilter === null ? ' active' : ''}`
  );
  allChip.type = 'button';
  ...
  for (const aud of audiences) {
    ...
    const chip = create(
      'button',
      `stacks-audience-chip${state.audienceFilter === aud ? ' active' : ''}`
    );
    chip.type = 'button';
