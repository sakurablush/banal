...
export function renderModelDetail(model: AIModel, lang: Lang, onBack?: () => void): HTMLElement {
  ...
  const privacyLevel =
    model.license.commercial && !model.license.restrictions?.length
      ? ('high' as const)
  ...
  if (alternatives.length > 0) {
    ...
    for (const alt of alternatives.slice(0, 3)) {
      ...
      recReason.textContent =
        alt.family === model.family
          ? lang === 'ja'
            ? '同じファミリー'
            : 'Same family'
          : lang === 'ja'
