 ...
 */
import { initI18n, getCurrentLang, t } from './i18n';
import { initTheme, setTheme } from './theme';
import { initDirectory } from './directory';
import { initHeroMesh } from './lib/hero-canvas';
...
// Support section: copy crypto addresses on click.
try {
  ...
  if (supportSection) {
    supportSection.addEventListener('click', (event) => {
      ...
      if (
        ...
        !target.classList.contains('support-crypto-copy')
      ) {
        return;
      }

      const address = target.dataset.copy;
      if (!address) return;

      const lang = getCurrentLang();
      const copiedLabel = t(lang, 'section.support.copied');
      const defaultLabel = t(lang, 'section.support.copy');
  ...
  // Non-critical — addresses remain selectable if clipboard fails.
}
