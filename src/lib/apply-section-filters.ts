...
export function applyModelsFilterValues(
  ...
  state.familyFilter =
    values.family && families.has(values.family) ? values.family : null;
  state.useCaseFilter =
    values.useCase && MODEL_USE_CASES.has(values.useCase) ? values.useCase : null;
  state.licenseFilter =
    values.license && licenses.has(values.license) ? values.license : null;
...
export function applyStacksFilterValues(
  ...
  state.audienceFilter =
    values.audience && valid.has(values.audience)
      ? (values.audience as StackAudience)
      : null;
}
...
export function applyZeroKeyFilterValues(
  ...
  state.activeCategory =
    values.cat && validCategories.has(values.cat)
      ? (values.cat as ZeroKeyCategory)
      : null;
  state.query = values.q ? values.q.slice(0, 200) : '';
...
export function applyPromptsFilterValues(
  ...
  state.selectedCategory =
    values.cat && validCategories.has(values.cat) ? values.cat : 'all';
}
