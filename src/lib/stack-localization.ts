...
function costBreakdownEqual(
  ...
  return a.every(
    (item, i) =>
      item.tool === b[i]?.tool && item.cost === b[i]?.cost && item.notes === b[i]?.notes
  );
}
...
function resourcesEqual(a: ToolStack['resources'], b: ToolStack['resources']): boolean {
  ...
  return a.every(
    (item, i) =>
      item.title === b[i]?.title && item.url === b[i]?.url && item.type === b[i]?.type
  );
}
...
function mergeCustomStack(baseEn: ToolStack, custom: CustomStack, lang: Lang): ToolStack {
  ...
  const costUnchanged =
    custom.cost.total === baseEn.cost.total && costBreakdownEqual(custom.cost.breakdown, baseEn.cost.breakdown);
