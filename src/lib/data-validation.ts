...
export function validateZeroKeyTool(tool: ZeroKeyTool): ValidationResult {
  ...
  if (!tool.bestFor || tool.bestFor.trim() === '') {
    warnings.push({
      field: 'bestFor',
      message: 'Tool bestFor description is recommended',
      severity: 'warning',
    });
  }
  ...
  if (!tool.qualityNote || tool.qualityNote.trim() === '') {
    warnings.push({
      field: 'qualityNote',
      message: 'Tool qualityNote is recommended',
      severity: 'warning',
    });
  }
  ...
  if (tool.badges && tool.badges.length > 10) {
    warnings.push({
      field: 'badges',
      message: 'Tool has too many badges (>10)',
      severity: 'warning',
    });
  }
...
export function validateTool(tool: Tool): ValidationResult {
  ...
  if (!tool.bestFor || tool.bestFor.trim() === '') {
    warnings.push({
      field: 'bestFor',
      message: 'Tool bestFor is recommended',
      severity: 'warning',
    });
  }
...
export function validateAIModel(model: AIModel): ValidationResult {
  ...
  if (!model.parameters || !model.parameters.total) {
    errors.push({
      field: 'parameters.total',
      message: 'Model parameters total is required',
      severity: 'error',
    });
  }
  ...
  if (!model.contextWindow || model.contextWindow.trim() === '') {
    errors.push({
      field: 'contextWindow',
      message: 'Model context window is required',
      severity: 'error',
    });
  }
  ...
  if (!model.license || !model.license.type) {
    errors.push({
      field: 'license.type',
      message: 'Model license type is required',
      severity: 'error',
    });
  }
  ...
  if (!model.hardware || !model.hardware.minRam) {
    errors.push({
      field: 'hardware.minRam',
      message: 'Model minimum RAM is required',
      severity: 'error',
    });
  }
  ...
  if (!model.bestFor || model.bestFor.length === 0) {
    warnings.push({
      field: 'bestFor',
      message: 'Model bestFor is recommended',
      severity: 'warning',
    });
  }
  ...
  if (!model.availability || model.availability.inferenceProviders.length === 0) {
    warnings.push({
      field: 'availability.inferenceProviders',
      message: 'Model has no inference providers',
      severity: 'warning',
    });
  }
  ...
  if (!model.availability || model.availability.localRunners.length === 0) {
    warnings.push({
      field: 'availability.localRunners',
      message: 'Model has no local runners',
      severity: 'warning',
    });
  }
...
export function validateToolStack(stack: ToolStack): ValidationResult {
  ...
  if (!stack.description || stack.description.trim() === '') {
    errors.push({
      field: 'description',
      message: 'Stack description is required',
      severity: 'error',
    });
  }
  ...
  if (!stack.audience || !stack.audience.type) {
    errors.push({
      field: 'audience.type',
      message: 'Stack audience type is required',
      severity: 'error',
    });
  }
  ...
  if (!stack.audience || !stack.audience.budget) {
    errors.push({
      field: 'audience.budget',
      message: 'Stack audience budget is required',
      severity: 'error',
    });
  }
  ...
  if (!stack.audience || !stack.audience.experience) {
    errors.push({
      field: 'audience.experience',
      message: 'Stack audience experience is required',
      severity: 'error',
    });
  }
  ...
  if (!stack.tools || stack.tools.length === 0) {
    errors.push({
      field: 'tools',
      message: 'Stack must have at least one tool',
      severity: 'error',
    });
  }
  ...
  if (!stack.workflow || stack.workflow.length === 0) {
    errors.push({
      field: 'workflow',
      message: 'Stack must have at least one workflow step',
      severity: 'error',
    });
  }
  ...
  if (!stack.cost || !stack.cost.total) {
    errors.push({
      field: 'cost.total',
      message: 'Stack total cost is required',
      severity: 'error',
    });
  }
  ...
  if (!stack.cost || !stack.cost.breakdown || stack.cost.breakdown.length === 0) {
    errors.push({
      ...
      message: 'Stack cost breakdown is required',
      severity: 'error',
    });
  }
  ...
  if (stack.workflow && stack.workflow.length < 3) {
    warnings.push({
      field: 'workflow',
      message: 'Stack has fewer than 3 workflow steps',
      severity: 'warning',
    });
  }
...
export function validateAllZeroKeyTools(tools: readonly ZeroKeyTool[]): ValidationResult {
  ...
  for (const tool of tools) {
    ...
    allWarnings.push(
      ...result.warnings.map((w) => ({ ...w, message: `[${tool.id}] ${w.message}` }))
    );
  }
...
export function validateAllAIModels(models: readonly AIModel[]): ValidationResult {
  ...
  for (const model of models) {
    ...
    allWarnings.push(
      ...result.warnings.map((w) => ({ ...w, message: `[${model.id}] ${w.message}` }))
    );
  }
...
export function validateAllToolStacks(stacks: readonly ToolStack[]): ValidationResult {
  ...
  for (const stack of stacks) {
    ...
    allWarnings.push(
      ...result.warnings.map((w) => ({ ...w, message: `[${stack.id}] ${w.message}` }))
    );
  }
