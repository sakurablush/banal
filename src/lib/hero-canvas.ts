...
export function initHeroMesh(root: ParentNode = document): () => void {
  ...
  // Three.js >= r163 requires WebGL 2. Try webgl2 first, fall back to webgl1
  // (which throws inside the renderer), and stay silent if neither is available
  // — the CSS gradient handles visuals.
  const gl =
