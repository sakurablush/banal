/**
 * Getting Started Guides copy — EN source of truth + JA mirror.
 * Rendered by components/getting-started-guides.ts.
 */
...
export interface GettingStartedGuidesCopy {
  title: string;
  subtitle: string;
    ...
    guides: [
      {
        ...
        steps: [
          ...
          '小さくコミット；無料モデルは存在しないimportやAPIを生成しやすい',
        ],
      },
      {
        id: 'local-ai-models',
...
} satisfies Record<Lang, GettingStartedGuidesCopy>;

export function getGettingStartedGuidesCopy(lang: Lang): GettingStartedGuidesCopy {
  return COPY[lang];
}

/** Guide ids and step counts per locale — for integrity tests. */
export function getGettingStartedGuidesMeta(lang: Lang): Array<{ id: string; stepCount: number }> {
  return COPY[lang].guides.map((g) => ({ id: g.id, stepCount: g.steps.length }));
}
