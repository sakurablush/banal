...
- **Hero animation is now a WebGL cosmic shader** (replaces the 2D horizon-mesh
  canvas). `src/lib/hero-mesh.ts` → `src/lib/hero-canvas.ts` + `src/lib/cosmicShader.ts`
  (ported from musicflow). Same `initHeroMesh(root) => cleanup` contract, so
  `src/main.ts` wiring is unchanged. Pauses off-screen, on tab hide, and under
  `prefers-reduced-motion`; falls back gracefully when WebGL is unavailable.
  Requires `three` (new dependency) and `@types/three` (devDependency).
...
- Bumped `@types/node` 20→26, `@typescript-eslint/*` 7.7→8.70, `tsx` 4.19→4.23,
  `js-yaml` 4.2→4.3. `three` added (production).
- Held back (intentional, pre-existing stability): `vitest`/`@vitest/coverage-v8`
  stay on 4.1.8 — vitest 5.x is pre-release only. `jsdom` stays on 24.x —
  jsdom 30 pulls undici 6.x that breaks our `undici ^5.28.5` override and
  re-introduces 11 High undici advisories. `eslint` stays on 8.x — 9.x is a
  breaking rewrite; 3 moderate brace-expansion advisories remain in eslint 8
  deps and are out of scope for this update.
